...
#Get Usuarios
@router.get('/', response_model=UsuarioSchemaBase)
...
async def get_delete(usuario_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        query: select(UsuarioModel).filter(UsuarioModel.id == usuario_id)
        result: await session.execute(query)
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_session)):
    ...
    return JSONResponse(content={"access-token": criar_token_acesso(sub=usuario.id),
                                 "token_type": "bearer"},
